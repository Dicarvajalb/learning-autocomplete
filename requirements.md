...
| AUTH-03 | The backend MUST expose an authentication entry point for Google login and exchange the verified Google identity for an application session. |
...
| AUTH-08 | Authenticated sessions MUST use signed application JWTs delivered through secure HTTP cookies or authorization headers. |
